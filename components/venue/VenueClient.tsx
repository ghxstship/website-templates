...
export function EventDetail({ event }: { event: VEvent }) {
  ...
  const checkout = async () => {
    if (count === 0) return;
    await captureBooking("venue", { kind: "ticket-hold", summary: `${count}× ${event.title} — ${VENUE.name}`, details: { event: event.title, date: event.date, count, total, settled_on: "ticketing" }, refPrefix: "ARM" });
    setConfirm({ title: "Tickets held", body: `${count} ticket(s) held for 10 minutes. Payment is completed on the FRONTROW ticketing network — follow the link in your email to pay and receive your mobile tickets.` });
    setQtys([0, 0, 0]);
  };
    ...
    <>
      ...
      <section className="wrap split2" style={{ paddingBlock: "16px clamp(40px, 5vw, 64px)", display: "grid", gridTemplateColumns: "minmax(0,1fr) minmax(0,1fr)", gap: "clamp(28px, 5vw, 64px)", alignItems: "start" }}>
        ...
        <div>
          ...
          <button type="button" className="btn btn-primary" onClick={checkout} disabled={count === 0} style={{ marginTop: 20, padding: "14px 26px", justifyContent: "flex-start" }}>Hold & pay on FRONTROW</button>
