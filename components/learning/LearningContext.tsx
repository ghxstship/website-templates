...
type Ctx = {
  ...
  toggleLesson: (slug: string, lessonIdx: number) => void;
...
export function LearningProvider({ children }: { children: React.ReactNode }) {
  ...
  const toggleLesson = useCallback((slug: string, lessonIdx: number) => {
    setDone((d) => {
  ...
  return (
    <LearningCtx.Provider value={{ enrolled, done, posts, likes, enroll, toggleLesson, isEnrolled: (s) => enrolled.has(s), doneCount: (s) => (done[s]?.size ?? 0), addPost, toggleLike, join }}>
      {children}
