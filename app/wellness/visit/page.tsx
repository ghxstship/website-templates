...
import { VISIT_INFO } from "@/lib/wellness";
export const metadata: Metadata = { title: "Visit" };
export default function VisitPage() {
  return (
    <div className="fadein">
      <PageHeader kicker="Plan your visit" title="Visit" />
      <section className="wrap split2" style={{ paddingBlock: "clamp(28px, 4vw, 56px)", display: "grid", gridTemplateColumns: "minmax(0, 1.1fr) minmax(0, 0.9fr)", gap: "clamp(32px, 5vw, 72px)", alignItems: "start" }}>
        <div>
          ...
          <div style={{ marginTop: 22 }}><Link href="/wellness/treatments" className="btn btn-primary" style={{ padding: "12px 20px", textDecoration: "none" }}>Book a treatment</Link></div>
        </div>
        <figure className="grayscale sticky-fig" style={{ margin: 0, aspectRatio: "3/4", position: "sticky", top: 96, border: "2px solid var(--color-divider)" }}><Placeholder label="Map / the spa" /></figure>
      </section>
    </div>
  );
}
